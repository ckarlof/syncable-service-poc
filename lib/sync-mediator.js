...
let SyncMediator = {
  ...
  init: function () {
    ...
    this.unsentQueues = {};
    this.sentItems = {};
    storage.revisionTable = storage.revisionTable || {};
    storage.seqNums = storage.seqNums || {};
    storage.seqNums[SyncDataTypes.HISTORY] = storage.seqNums[SyncDataTypes.HISTORY] || 0;
    return this;
  },

  start: function () {
    let self = this;
    let dataType = SyncDataTypes.HISTORY;
    let syncableService = self.syncableServices[dataType];
    self.initPouch(function (err) {
      if (!err) {
        syncableService.mergeDataAndStartSyncing(dataType, [], self);
        self.listenForUpstreamChanges();
      }
    });
    return;
  },

  initPouch: function(callback) {
    let self = this;
  ...
  listenForUpstreamChanges: function() {
    let self = this;
    self.db.changes({
      include_docs: true,
      since: storage.seqNums[SyncDataTypes.HISTORY],
      continuous: true,
  ...
  processDownstreamChangeFromCouch: function(dataType, change) {
    storage.seqNums[dataType] = change.seq;
    ...
    // update rev table
    storage.revisionTable[change.id+"-"+change.doc.contentRevision] = _rev;
    //L.log("Format downstream change revision table", storage.revisionTable, change);
    if (sentItem &&
  ...
  formatUpstreamChangeForCouch: function(dataType,specifics) {
    ...
    if (typeof parentContentRevision !== "undefined") {
      L.log("revision", parentContentRevision, storage.revisionTable);
      couchChange._rev = storage.revisionTable[specifics._id+"-"+parentContentRevision];
    }
    return couchChange;
  },
