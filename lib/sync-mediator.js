...
let SyncMediator = {
  ...
  processDownstreamChangeFromCouch: function(dataType, change) {
    ...
    // update rev table
    this.revisionTable[change.id+"-"+change.doc.contentRevision] = _rev;
    L.log("Format downstream change revision table", this.revisionTable, change);
    if (sentItem &&
  ...
  formatUpstreamChangeForCouch: function(dataType,specifics) {
    ...
    if (typeof parentContentRevision !== "undefined") {
      L.log("revision", parentContentRevision, this.revisionTable);
      couchChange._rev = this.revisionTable[specifics._id+"-"+parentContentRevision];
    }
    return couchChange;
  },
