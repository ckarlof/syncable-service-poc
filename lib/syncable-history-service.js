...
let SyncableHistoryService = {
  ...
  mergeDataAndStartSyncing: function (dataType, listOfSyncData, syncChangeProcessor) {
    ...
    self.syncChangeProcessor = syncChangeProcessor;
  ...
  applySyncChangesToLocalService: function (changes) {
    let self = this;
    let deletions = changes.filter(function (change) { return change.isDelete(); });
    let updates = changes.filter(function (change) { return change.isAdd() || change.isUpdate(); });
    return group([
