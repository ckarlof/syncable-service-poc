...
storage.contentRevisionMap = storage.contentRevisionMap || {};
...
function getContentRevisionEntry(guid) {
  let contentRevisionEntry = storage.contentRevisionMap[guid];
  if (!contentRevisionEntry) {
    contentRevisionEntry = { contentRevision: 0 };
    storage.contentRevisionMap = contentRevisionEntry;
  }
  return contentRevisionEntry;
}

function incrementContentRevisionEntry(guid) {
  let contentRevision = (storage.contentRevisionMap[guid] || {}).contentRevision;
  ...
  contentRevision++;
  //L.log("incrementing content revision", guid, contentRevision, parentContentRevision)
...
function updateContentRevisionEntry(guid, contentRevision, parentContentRevision) {
  storage.contentRevisionMap[guid] = { contentRevision: contentRevision, parentContentRevision: parentContentRevision };
}
...
var historyObserver = {
  ...
  onDeleteVisits: function (uri, visitTime, guid, reason) {
    if (this.trackingCountdownLatch > 0) {
      return;
    }
...
var HistoryService = {
  ...
  updateItems: function(historyItems) {
    var self = this;
    if (self.historyObserver) {
      self.historyObserver.trackingCountdownLatch++;
    }
    L.log('Saving', historyItems.length, 'items to local database');
    return group(historyItems.map(function (historyInfo) {
      let historyItem = HistoryItem(historyInfo);
      // Update content revisions first before writing to Places. In a
      // real implementation, you would store the content revisions in
      // Places.
